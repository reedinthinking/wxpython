...
class Scount(object):
    def __init__(self):
        ...
        self.log = self.getLogger()#Logger(self.getAbsolutePath() + '/../log/operate.log', level=self.logLevel)

    def getLogger(self):
        logLevel = self.getConfigContent('config')['logLevel']
        logfile = self.getAbsolutePath() + '/../log/operate-'+datetime.datetime.now().strftime("%Y-%m-%d")+'.log'
    ...
    def annotation(self,ref_seq,blast_ident,run_path):
        ...
        logstr = "*Step10--打开文件：%s/%s"  % (run_path,ref_seq)
        ...
        #print lenth_r
        scmd0 = ('%s -query %s/final.contigs.fa -out %s/s_%d_out -db %s/Sdb -outfmt 6  -num_threads 2 -perc_identity %d %s' %(self.blastn,run_path,run_path,blast_ident, run_path,blast_ident,outfile))
        os.system(scmd0)
        logstr = "*Step10--执行命令：%s" % (scmd0)
        self.log.logger.info(logstr)
        print logstr

        fr2 = open(r'%s/s_%d_out' %  (run_path,blast_ident), 'r')
        logstr = "*Step10--打开文件：%s/s_%d_out" % (run_path,blast_ident)
        self.log.logger.info(logstr)
        print logstr
